...
defmodule StrangertalksNew.ConversationLifecycle.MemoriesTest do
  ...
  @valid_attrs %{
    ...
    memory_content: "Reflecting on how step-by-step structural isolation saves hours of debugging.",
  ...
  setup do
    ...
    p_a = struct(StrangertalksNew.Participant, %{
      participant_id: Ecto.UUID.generate(),
      presence_state: :ONLINE
    }) |> StrangertalksNew.Repo.insert!()

    p_b = struct(StrangertalksNew.Participant, %{
      participant_id: Ecto.UUID.generate(),
      presence_state: :ONLINE
    }) |> StrangertalksNew.Repo.insert!()
    ...
    match = %StrangertalksNew.Matching{}
            |> StrangertalksNew.Matching.changeset(match_attrs)
            |> StrangertalksNew.Repo.insert!()

    # 3. Build conversation records cleanly using context wrappers
    {:ok, conversation} = StrangertalksNew.ConversationLifecycle.Conversations.create_conversation(%{
