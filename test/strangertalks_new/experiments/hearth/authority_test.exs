defmodule StrangertalksNew.Experiments.Hearth.AuthorityTest do
  ...
  test "room routing increments structural turns without accepting or retaining message bodies", %{authority: authority} do
  ...
  test "explicit leave dissolves room immediately and returns only structural partner effects", %{authority: authority} do
    room_id = create_room(authority)
  ...
  test "pass and disconnect dissolve a bridge and identify the neutral survivor reset", %{authority: authority} do
  ...
  defp create_room(authority) do
    ...
    {:ok, %{status: :room_ready, room_id: room_id}} = Authority.step_in(authority, "bob", bridge_id)
    room_id
  end
end
