defmodule StrangertalksNew.Team4DbSecurityClosureTest do
  ...
  test "security inventory covers every current public application table and RLS is enabled" do
    rows = existing_application_tables()
    table_names = Enum.map(rows, &hd/1)

    assert table_names == Enum.sort(@application_tables),
           "public application-table inventory changed; update the security closure explicitly"
  ...
  test "Supabase API roles have no table privileges on application tables" do
    existing_roles = existing_roles(@api_roles)

    for role <- existing_roles,
        table <- @application_tables,
