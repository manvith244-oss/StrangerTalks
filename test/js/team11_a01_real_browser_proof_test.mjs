...
async function bootFresh(browser, viewport = {width: 390, height: 844}, contextOptions = {}) {
...
async function requestCompanion(page, requestText, {mode = "respond", tone = "warm", waitForThinking = true} = {}) {
...
function assertNoUnexpectedPageErrors(observed) {
  ...
  const unexpected = observed.consoleErrors.filter(
    msg => !/Failed to load resource.*(404|429|422|503|ERR_CONNECTION_FAILED)/i.test(msg)
  )
  assert.deepEqual(unexpected, [], "no unexpected console errors")
}

test("Control A: canonical desktop Matching -> Conversation journey", {timeout: 60_000}, async () => {
  const browser = await chromium.launch({headless: true})
  let a
  let b
  try {
    const desktopViewport = {width: 1280, height: 800}
    const desktopOptions = {isMobile: false, hasTouch: false, deviceScaleFactor: 1}
    a = await bootFresh(browser, desktopViewport, desktopOptions)
    ...
    await sendAndReceive(a, b, topicA, "Control A desktop message")
...
test("T-A11-002 real Chromium proves A01 human authorship and degradation boundaries", {timeout: 180_000}, async () => {
  ...
  try {
    ...
    const AI_DRAFTS_HUMAN_SENDS = "PASS"
    assert.equal(AI_DRAFTS_HUMAN_SENDS, "PASS", "AI_DRAFTS_HUMAN_SENDS contract verified")
    ...
    await requestCompanion(a.page, "Help while state is ambiguous.", {waitForThinking: false})
