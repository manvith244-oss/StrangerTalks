defmodule StrangertalksNew.WT02ContextRecorder do
  ...
  def init(opts) when is_list(opts) do
    case start_link(opts) do
      ...
      {:error, {:already_started, recorder}} -> {:stop, {:stale_recorder, recorder}}
    end
  end
  ...
  defp write_artifact(s, t) do
    ...
    valid = valid_pre_reset?(Map.get(c, :pre_reset)) and valid_after_setup?(Map.get(c, :after_setup)) and
      complete and valid_post_race?(Map.get(c, :post_race)) and is_nil(Map.get(c, :observation_error))
  ...
  defp valid_pre_reset?(%{monotonic: monotonic, queue_state: queue_state}) when is_integer(monotonic) and is_map(queue_state), do: true
  defp valid_pre_reset?(_), do: false
  defp valid_after_setup?(%{queue_state_entry_count: count, queue_state_expected_empty: expected_empty}) when is_integer(count) and is_boolean(expected_empty), do: true
