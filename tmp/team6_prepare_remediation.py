...
# Preserve valid boundaries when the direct script inserts server helpers/gates and tests.
text = text.replace(
...
text = text.replace(
    ...
    '    server = replace_once(server, admitted_marker, helpers + "\\n\\n" + admitted_marker, "authoritative media helpers")',
    1,
)
text = text.replace(
