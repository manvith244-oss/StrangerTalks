...
const FOUR_DOOR_INTENTS = new Set(DOORS.map(({value}) => value))
const TALK_LANGUAGE_CODES = new Set(CONVERSATION_LANGUAGES.map(({value}) => value))
...
const EVENT_SCHEMAS = Object.freeze({
  ...
  st_talk_language_opened: Object.freeze({
    ...
    properties: Object.freeze({trigger: (value) => TALK_LANGUAGE_OPEN_TRIGGERS.has(value)})
  }),
  ...
  st_first_message_accepted: Object.freeze({required: Object.freeze([]), properties: Object.freeze({})}),
...
function isIntentCode(value) { return FOUR_DOOR_INTENTS.has(value) }
function isTalkLanguageCode(value) { return TALK_LANGUAGE_CODES.has(value) }
...
function sanitizeEvent(name, properties, flowAttemptId, testTraffic) {
  ...
  for (const property of schema.required) if (!Object.hasOwn(sanitized, property)) return null
...
export function createProductEventTracker(options = {}) {
  ...
  return {get flowAttemptId() { return flowAttemptId }, capture, captureOnce, resetFlow}
}

export function createIntentSelectionObserver(tracker) {
  let selectedIntent = null
  let queueJoined = false
  const syncFlow = createFlowSynchronizer(tracker, () => { selectedIntent = null; queueJoined = false })
  return {
    async select(intentValue) {
      syncFlow()
      if (queueJoined || !isIntentCode(intentValue) || intentValue === selectedIntent) return false
      if (selectedIntent === null) {
        selectedIntent = intentValue
        return tracker.capture("st_intent_selected", {intent_family: "four_doors", intent_value: intentValue, selection_kind: "first"})
      }
      const fromIntent = selectedIntent
      selectedIntent = intentValue
      return tracker.capture("st_intent_changed", {from_intent: fromIntent, to_intent: intentValue})
    },
    markQueueJoined() { syncFlow(); queueJoined = true }
  }
}
...
export function createTalkLanguageObserver(tracker) {
  ...
  const syncFlow = createFlowSynchronizer(tracker, () => { currentLanguage = null; rememberedRecorded = false })
  return {
    opened(trigger) {
      syncFlow()
    ...
    async selected(languageCode, validLanguages) {
      syncFlow()
      if (!validLanguageCode(languageCode, validLanguages) || currentLanguage === languageCode) return false
  ...
  return {
    ...
    joined() {
      ...
      const capturePromise = (async () => {
        if (!(await prerequisite) || tracker.flowAttemptId !== flowAttemptId || requestCapturePromise !== prerequisite) return false
        const recorded = await tracker.captureOnce("st_queue_joined", {intent_code: capturedIntentCode})
        if (recorded && tracker.flowAttemptId === flowAttemptId && joinedCapturePromise === capturePromise) queueJoinedInFlow = true
        return recorded
      })()
    ...
    matched() {
      ...
      const capturePromise = (async () => {
        ...
        if (tracker.flowAttemptId !== flowAttemptId || !queueJoinedInFlow) return false
        const recorded = await tracker.captureOnce("st_match_created", {intent_code: capturedIntentCode})
        if (recorded && tracker.flowAttemptId === flowAttemptId && matchedCapturePromise === capturePromise) matchedInFlow = true
        return recorded
      })()
...
export function createEntranceAttemptCoordinator(tracker) {
  ...
  return {
    entranceReady(options = {}, {newAttempt = false} = {}) {
      if (newAttempt && (lastEntranceFlowAttemptId === null || tracker.flowAttemptId === lastEntranceFlowAttemptId)) tracker.resetFlow()
...
export const productEvents = createProductEventTracker({sink: runtimeSink, testTraffic: globalThis.__strangerTalksTestTraffic === true})
