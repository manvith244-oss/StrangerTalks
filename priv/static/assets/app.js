...
function initializeLifetimePresentation() {
  ...
  reportForm.insertAdjacentHTML("afterbegin", '<div class="report-disclosure"><h2>About this report</h2><p>Reports are stored separately for safety and can remain after this Conversation ends.</p><p>An unsent text message may be kept temporarily inside the active Conversation for safety reporting. If that unsent message is reported while the safety copy is still available, the specific message text may be stored with the report.</p><p>Submitting this report does not automatically save the ordinary Conversation transcript, Reply structure, GIF or sticker choice, voice-note identity or audio, or surrounding chat history into the report.</p><p>Limited safety evidence is minimized or deleted on its own safety-retention schedule. Unsend or Fade can remove participant-visible or local content, but they do not erase safety evidence already authorized for safety handling.</p><p id="report-target-disclosure" role="status" hidden></p></div>')
