...
function activeScreen(documentRef) { return documentRef.querySelector("[data-screen].active") }
...
export function installArrivalFirstMinute(documentRef = globalThis.document, windowRef = globalThis.window) {
  ...
  const restoreQueueCopy = () => { if (queueLede) queueLede.textContent = `Looking for someone who chose ${queueDoorLabel()} in ${queueLanguageLabel()}.` }
  const setDoorBusy = (busy) => { joinInFlight = busy; doorGrid.setAttribute("aria-busy", busy ? "true" : "false") }
  const ensureStartupFailure = () => {
    ...
    retry.type = "button"; retry.textContent = "Retry"; retry.addEventListener("click", () => windowRef.location.reload())
    panel.append(title, copy, retry); trustCue.after(panel); languageSelect.disabled = true
  ...
  doorGrid.addEventListener("click", (event) => {
    ...
    if (!languageSelect.value) {
      ...
      event.preventDefault(); event.stopImmediatePropagation(); setDoorBusy(false)
      ...
      languageSelect.focus(); return
    }
    if (joinInFlight) { event.preventDefault(); event.stopImmediatePropagation(); return }
    clearFeedback(); setDoorBusy(true)
  }, true)
  languageSelect.addEventListener("pointerdown", () => { void talkLanguageEvents.opened("direct") })
  languageSelect.addEventListener("keydown", (event) => {
    if (!event.repeat && DIRECT_LANGUAGE_KEYS.has(event.key)) void talkLanguageEvents.opened("direct")
  })
  ...
  leaveQueue?.addEventListener("click", () => {
    queueMicrotask(() => {
      ...
      leaveQueue.disabled = true; leaveQueue.setAttribute("aria-busy", "true")
    })
  }, true)
  const resetLeaveQueue = () => { if (leaveQueue) { leaveQueue.disabled = false; leaveQueue.removeAttribute("aria-busy") } }
  const scheduleScreenFocus = () => {
    ...
    windowRef.requestAnimationFrame(() => {
      ...
      if (!screen) return
      if (screen.dataset.screen === "doors") { setDoorBusy(false); resetLeaveQueue() }
      if (screen.dataset.screen !== "queue") resetLeaveQueue()
      if (!focusIsInsideScreen(documentRef, screen)) focusScreenHeading(screen)
    })
  }
  const screenObserver = new MutationObserver(() => scheduleScreenFocus())
  documentRef.querySelectorAll("[data-screen]").forEach((screen) => screenObserver.observe(screen, {attributes: true, attributeFilter: ["class"]}))
  const handleStatus = () => {
    const message = status.textContent.trim(); const screenName = activeScreen(documentRef)?.dataset.screen
    if (message === FIRST_MINUTE_FAILURE) {
      setDoorBusy(false); setFeedback("StrangerTalks could not start. Matching has not begun.")
      ensureStartupFailure().querySelector("button")?.focus(); return
    }
    if (message === "Connecting…" && screenName === "queue") { if (queueLede) queueLede.textContent = "Connecting securely before matching starts…"; return }
    if (message === "Connection interrupted. Reconnecting." && screenName === "queue") { if (queueLede) queueLede.textContent = "Connection interrupted. Reconnecting before matching continues…"; return }
    if (message === "Queue status: queued") { restoreQueueCopy(); return }
    if (message === "Could not cancel matching. Please try again." && screenName === "queue") { if (queueLede) queueLede.textContent = message; resetLeaveQueue(); return }
    if (message === "Could not start matching right now. Please try again." && screenName === "doors") { setDoorBusy(false); setFeedback(message) }
  }
  const statusObserver = new MutationObserver(handleStatus)
  statusObserver.observe(status, {childList: true, characterData: true, subtree: true})
  return {clearFeedback, handleStatus, scheduleScreenFocus}
}

if (typeof document !== "undefined" && typeof window !== "undefined") installArrivalFirstMinute(document, window)
